...
ggplot(data=diamonds)+ geom_density(aes(x=price, color=carat, fill=carat),alpha=0.3)
ggplot(data=diamonds)+ geom_jitter(aes(x=carat, y=price, color=carat, fill=carat),alpha=0.3)
ggplot(data=diamonds)+ geom_violin(aes(x=carat, y=price, color=carat, fill=carat),alpha=0.3)
ggplot(data=diamonds)+ geom_boxplot(aes(x=carat, y=price, color=carat, fill=carat),alpha=0.3)
...
```{r}
Preis.aov<-aov(price ~ carat+cut+color+clarity+depth+table+x+y+z, data=diamonds)
Preis.aov
summary(Preis.aov)
```

Die ANOVA zeigt, dass Karat, Schnitt, Farbe, Reinheit, table und x einen hochsignifikanten Einfluss auf den Preis haben (***). Der Einfluss von depth auf den Preis ist auch signifikant auf dem Level ‘*’ 0.05. Von y und z hingegen scheint der Preis nicht abhängig zu sein. Die Werte sind nicht signifikant.
...
Der Diamant wäre vermutlich ca. 38,7 Mio. US-Dollar wert.
