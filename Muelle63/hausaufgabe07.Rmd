...
Die rote Verteilung sieht am fairsten aus.Die Kurve sieht normalverteilt aus und erfasst im Mittelfeld eine große Menge der Studenten (Fläche unter der Kurve). Die Wahrscheinlichkeit für Durchfallen ist niedriger als bei den anderen Standardabweichungen.
...
```{r}
for(s in c(3,4,5) ){
  durchfall <- pnorm(13,mean=mu,sd=s)
  output <- paste("Bei einer Standabweichung von",s, "",durchfall*100,"% durch.")
  print(output)
}
```
...
Es gibt eine viel höhere Wahrscheinlichkeit für schlechte Noten und eine geringere für ausgezeichnete Noten.

Warum?
Die schlechten Noten gehen von 0-5 Punkten, die guten hingegen nur von 13-15. Die schlechten Noten haben somit mehr Werte und eine größere Auftretenswahrscheinlichkeit.
...
antwort_hier
Bei der Notenpunktverteilung (von 0-15 Punkte) liegt der Gipfel der guten Noten rechts. Aus diesem Grund ist die Verteilung linksschief.
...
Man sollte einen zweiseitigen z-Test machen, da der Unterschied in beide Richtungen gehen kann.

In einem kleinen Seminar mit 7 Studenten beträgt der Durchschnittswert 10. Unterscheidet sich der Notenspiegel von dem idealen? Berechnen Sie den $z$-Test: ungerichter zweiseitiger z-Test

```{r}

z7Studenten <- sqrt(7)*((10-8)/3)

print(z7Studenten)

```

Das ist ein **_insignifikanter_** Unterschied, denn 1.76 < 1.88.
...
z50Studenten <- sqrt(50)*((10-8)/3)

print(z50Studenten)

```

Das ist ein **_signifikanter_** Unterschied, denn 4.71 > 1.88. 
...
z20Studenten <- sqrt(20)*((7-8)/3)

print(z20Studenten)

```

Das ist ein **_insignifikanter_** Unterschied, denn |1.49| < 1.55. 
...
z25Studenten <- sqrt(25)*((7-8)/3)

print(z25Studenten)

```

Das ist ein **_signifikanter_** Unterschied, denn |1.67| > 1.55.
